...
<body>
  <div class="container">
    <div class="header">
      <h1 style="margin: 0; font-size: 24px;">🎯 ${isArabic ? "حجز جديد من الشات بوت" : "New Chatbot Booking"
    }</h1>
    ...
    <div class="content">
      ...
      <div class="section">
        ...
        <div class="detail-row">
          ...
          <span class="value"><a href="mailto:${userInfo.email}">${userInfo.email
    }</a></span>
        </div>
        <div class="detail-row">
          <span class="label">${isArabic ? "الهاتف:" : "Phone:"}</span>
          <span class="value"><a href="tel:${userInfo.phone}">${userInfo.phone
    }</a></span>
        </div>
        <div class="detail-row">
          <span class="label">${isArabic ? "اللغة المفضلة:" : "Preferred Language:"
    }</span>
          <span class="value">${userInfo.preferredLanguage === "ar" ? "العربية" : "English"
    }</span>
        </div>
      </div>
      ...
      ? `
          <div class="detail-row">
            ...
            <span class="value">${tripData.destination.name || tripData.destination
      }</span>
          </div>
        `
      : ""
      ...
      ? `
          ...
          <div class="detail-row">
            ...
            <span class="value">${tripData.dates.nights} ${isArabic ? "ليالي" : "nights"
      }</span>
          </div>
        `
      : ""
      ...
      ? `
        ...
        ? `+ ${tripData.travelers.children} ${isArabic ? "طفل" : "child(ren)"
        }`
        : ""
      }
            </span>
        ...
        : ""
          ...
          <div class="detail-row">
            ...
            <span class="value" style="color: #019fb1; font-weight: 700;">
              ${tripData.hotel.price_egp?.toLocaleString()} ${isArabic ? "ج.م" : "EGP"
    ...
    <div class="footer">
      ...
      <p style="margin: 5px 0 0 0;">${isArabic ? "نظام الحجز الذكي" : "Smart Booking System"
    }</p>
    </div>
  </div>
</body>
  ...
  <div class="container">
    ...
    <div class="content">
      <div class="success-box">
        <h2 style="margin: 0 0 10px 0; color: #10b981; font-size: 22px;">✅ ${isArabic ? "تم استلام طلبك!" : "Request Received!"
    }</h2>
      ...
      ? `
        <div class="section">
          <div class="section-title">📍 ${isArabic ? "تفاصيل الرحلة" : "Trip Details"
      }</div>
          <div class="detail-row">
            <span class="label">${isArabic ? "الوجهة" : "Destination"}</span>
            <span class="value">${tripData.destination.name || tripData.destination
        ...
        ? `
            <div class="detail-row">
              ...
              <span class="value">${tripData.dates.startDate} - ${tripData.dates.endDate
        } (${tripData.dates.nights} ${isArabic ? "ليالي" : "nights"
        })</span>
            </div>
          `
        : ""
        ...
        ? `
            <div class="detail-row">
              <span class="label">${isArabic ? "عدد المسافرين" : "Number of Travelers"
        }</span>
              <span class="value">${tripData.travelers.total} ${isArabic ? "مسافر" : "traveler(s)"
        }</span>
            </div>
          `
        : ""
      ...
      ? `
        <div class="section">
          <div class="section-title">🏨 ${isArabic ? "الفندق المختار" : "Selected Hotel"
      }</div>
          <div class="detail-row">
            <span class="label">${isArabic ? "الفندق" : "Hotel"}</span>
            <span class="value">${isArabic
        ...
        ? `
            <div class="detail-row">
              <span class="label">${isArabic ? "نظام الوجبات" : "Meal Plan"
        }</span>
              <span class="value">${tripData.mealPlan.label}</span>
            </div>
          `
        ...
        ? `
            <div class="detail-row">
              <span class="label">${isArabic ? "نوع الغرفة" : "Room Type"
        }</span>
              <span class="value">${tripData.roomType.label}</span>
            </div>
          `
    ...
    <div class="footer">
      ...
      <p style="margin: 5px 0 0 0;">${isArabic ? "رحلات لا تُنسى" : "Unforgettable Journeys"
    }</p>
    </div>
  </div>
</body>
